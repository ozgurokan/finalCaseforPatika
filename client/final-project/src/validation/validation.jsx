...
const yearRegex = /^[0-9]+$/;
export const newCarValidations = yup.object().shape({
    brand : yup.string().required(reqmes),
    model : yup.string().required(reqmes),
    plate : yup.string().matches(plateRegex).required(reqmes) ,
    year : yup.string().matches(yearRegex).required(reqmes)
})